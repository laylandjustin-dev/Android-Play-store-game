...
class GatheringTraitTest {
    ...
    fun `a base-three people gather and build at exactly the old rate`() {
        ...
        // Gathering existed, and they all still mean what they say.
        val base = TraitAllocation.of(3, 4, 3, 4, 8)
        assertEquals(TraitConfig.BASE_VALUE, base[Trait.GATHERING])
    ...
    @Test
    fun `gathering moves gathering and building in both directions`() {
        val low = TraitAllocation.of(3, 4, 3, 4, 5, 1)
        val high = TraitAllocation.of(3, 4, 3, 4, 5, 8)
        assertTrue(low.gatherYield < 1.0 && low.buildRate < 1.0, "a Gathering-1 people were not worse off")
        assertTrue(high.gatherYield > 1.0 && high.buildRate > 1.0, "a Gathering-8 people were not better off")
    }

    @Test
    fun `a gathering people really does gather more wood`() {
        fun woodAt(gathering: Int): Double {
            val sim = Simulation.newRun(
                RunConfig(seed = 909L, traits = TraitAllocation.of(3, 4, 3, 4, 5, gathering), civCount = 1),
            )
        ...
        assertTrue(
            loggers > plain,
            "a Gathering-8 people gathered $loggers against a base people's $plain",
        )
    }

    @Test
    fun `five values still build a legal allocation, with gathering at base`() {
        val five = TraitAllocation.of(5, 5, 5, 5, 5)
        assertEquals(TraitConfig.BASE_VALUE, five[Trait.GATHERING])
    ...
    fun `the sixth trait survives a save`() {
        ...
        assertEquals(8, reloaded.civ(0).traits[Trait.GATHERING])
    ...
    fun `every personality has an appetite for the sixth trait`() {
        ...
        // Gathering would never be drawn. This is the test that would notice.
        val rng = SimRandom(5L)
        for (personality in Personality.entries) {
            val start = TraitAllocation.of(4, 4, 4, 4, 4, 4)
            val drew = (0 until 3_000).any {
                RivalStrategist.chooseGrowth(personality, start, rng) == Trait.GATHERING
            }
